...
for _, row in df.iterrows():#fileN, path in enumerate(input_files_db['path'].to_list()):
    ...
    for i in range(np.ceil(total_events / events_per_job)):
    ...
    if debug:
        ...
        iter_data = [i.update({'nEvents':10}) for i in iter_data]
