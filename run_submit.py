...
for _, row in input_files_db.iterrows():#fileN, path in enumerate(input_files_db['path'].to_list()):
    ...
    job_template = {
        ...
        "arguments": f"{path_to_fs} $(input_file_name) $(start_event) $(nEvents)",          # we will pass in the value for this macro via itemdata
        ...
        'environment' :f'"out_dir={os.path.join(output_dir, run_tag, str(fid))}/$(subjob) extra_fs_args={extra_fs_args}"'# inputFile={lPath}"'#SHIP_CVMFS_SETUP_FILE=$ENV(SHIP_CVMFS_SETUP_FILE) FAIRSHIP_DIR=$ENV(FAIRSHIP) MAGNET_GEO=$ENV(MAGNET_GEO)"
